 ...
 *       device.N.name|mac|broadcast|port|mode|modeValue}（N 从 1 起，mode 空=普通广播）；兼容旧单设备键 {@code
 *       device.mac|broadcast|port}（自动迁移）
 *   <li>{@code settings.properties}（UTF-8）：软件设置，键 {@code ui.theme}（主题 id）、{@code ui.language}（语言
 *       code）、{@code device.count}（1-100）与 {@code mod.enabled.<id>}（true/false）
 * </ul>
 *
...
public final class ConfigService {
  ...
  /** settings.properties 键：主题标识（dark|light 或外部主题 id） */
  private static final String KEY_THEME = "ui.theme";

  /** settings.properties 键：语言代码（zh-CN 等） */
  private static final String KEY_LANGUAGE = "ui.language";

  /** settings.properties 键前缀：启用插件（mod.enabled.<id>=true/false） */
  private static final String MOD_ENABLED_PREFIX = "mod.enabled.";
  ...
  /** 发送模式标识键（空 = 普通广播，非空 = SendMode id） */
  private static final String KEY_MODE = ".mode";

  /** 模式专属数据键 */
  private static final String KEY_MODE_VALUE = ".modeValue";
  ...
  /** 设备键识别正则：组 1 为编号，组 2 为属性名（含旧单设备键，兼容迁移） */
  private static final Pattern DEVICE_INDEX_PATTERN =
      Pattern.compile("^device\\.(\\d+)\\.(name|mac|broadcast|port|mode|modeValue)$");
   ...
   *     AppConfig#DEFAULT_THEME}、语言 {@link AppConfig#DEFAULT_LANGUAGE}），sendCount 恒在 1-100
   */
  public static AppSettings loadSettings() {
    ...
    if (Files.exists(file)) {
      ...
      try (InputStream in = Files.newInputStream(file)) {
        ...
        settings.setLanguage(props.getProperty(KEY_LANGUAGE));
        for (String key : props.stringPropertyNames()) {
          if (key.startsWith(MOD_ENABLED_PREFIX)
              && Boolean.parseBoolean(props.getProperty(key))) {
            settings.setModEnabled(key.substring(MOD_ENABLED_PREFIX.length()), true);
          }
        }
      } catch (IOException e) {
   ...
   *
   * <p>数据契约：键 {@code device.N.name|mac|broadcast|port|mode|modeValue}（N 从 1 连续编号，与列表下标对应）； port
   * 为十进制字符串，mode 为空串表示普通广播。值仅转义反斜杠——'=' 出现在值中是安全的（Properties 按首个 '=' 切分键值），换行由单行输入框排除。
   *
  ...
  private static void writeDevices(Writer writer, DeviceConfig config) throws IOException {
    ...
    for (int i = 0; i < devices.size(); i++) {
      ...
      writeEntry(writer, DEVICE_KEY_PREFIX + index + KEY_MODE, d.getMode());
      writeEntry(writer, DEVICE_KEY_PREFIX + index + KEY_MODE_VALUE, d.getModeValue());
      writer.write("\n");
    }
  }

  /**
   * 写出软件设置：device.count、ui.theme、ui.language 与 mod.enabled.* 键。
   *
   * @throws IOException 写入失败时
   */
  private static void writeSettings(Writer writer, AppSettings settings) throws IOException {
    ...
    writeEntry(writer, KEY_LANGUAGE, settings.getLanguage());
    for (String modId : settings.getEnabledMods()) {
      writeEntry(writer, MOD_ENABLED_PREFIX + modId, "true");
    }
  }
  ...
  /**
   * 确保插件相关目录存在：{@code mods}（插件）、{@code resources}（主题）、{@code i18n}（语言）。
   *
   * <p>启动时调用，便于用户直接往目录里丢 jar。副作用：目录创建（I/O）；失败仅告警不阻断。
   */
  public static void ensurePluginDirs() {
    Path base = getConfigDir();
    for (String name :
        new String[] {
          AppConfig.MODS_DIR_NAME, AppConfig.RESOURCES_DIR_NAME, AppConfig.I18N_DIR_NAME
        }) {
      try {
        Files.createDirectories(base.resolve(name));
      } catch (IOException e) {
        log.warn("创建插件目录失败: {} ({})", name, e.toString());
      }
    }
  }
   ...
   * 解析属性键值对为设备列表。
   *
   * <p>数据契约（Properties 的键 → 值类型）：{@code device.N.name|mac|broadcast|mode|modeValue} → String；{@code
   * device.N.port} → String 十进制端口（非法回退默认 9）。 键缺失的属性保留模型默认值；编号乱序时按升序收敛为连续列表。
   *
        ...
        case "port" ->
            d.setPort(
                parseRange(
                    ...
                    AppConfig.PORT_MAX,
                    "端口"));
        case "mode" -> d.setMode(props.getProperty(key));
        case "modeValue" -> d.setModeValue(props.getProperty(key));
      }
    }
