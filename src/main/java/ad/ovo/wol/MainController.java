 ...
 *
 * <p>分层约定：网络发送委托 {@link WolService}，持久化委托 {@link ConfigService}，
 * 本类只翻译界面状态与用户意图，不直接触碰 UDP 与配置文件。
 *
 * <p>可变状态（均在 FX 线程内读写）：{@link #devices} 设备列表（持久化的唯一
 * 事实来源）、{@link #currentDevice} 当前编辑设备、{@link #dirty} 未保存标记、
 * {@link #currentTheme} 当前主题。
 *
 * <p>线程模型：唯一后台线程为发送线程 {@code wol-send-thread}（见
 * {@link #onSend()}），其结果经 Task 回调回到 FX 线程；其余方法一律在 FX 线程执行。
 *
...
public class MainController {
     ...
     *
     * <p>数据契约：{@link ConfigService#load()} 保证列表非空（至少一台默认设备）；
     * {@link ConfigService#loadSettings()} 保证主题/次数为合法值（非法回退默认）。
     *
     * <p>执行顺序依赖：本阶段 {@code statusBanner.getScene()} 为 null，
     * {@link #applyTheme(String)} 只同步按钮图案，不触碰场景样式表。
     *
     ...
     *
     * <p>约束：表单无需先保存即可发送；发送期间禁用全部操作按钮防止重复提交，
     * 结果（成功/失败/取消）回显状态后统一恢复按钮。
     *
     * <p>副作用：发起 UDP 网络发送（后台线程，阻塞时长约为连发次数 × 100ms），
     * 不修改配置与设备对象。
     */
    @FXML
    private void onSend() {
     ...
     *
     * <p>副作用：写盘两个配置文件（I/O，原子写入）；写盘失败时内存中的设备值
     * 已被表单覆盖（不回滚），界面提示失败原因。
     */
    @FXML
    private void onSave() {
     ...
     * 「删除」：移除当前设备并落盘。
     *
     * <p>不变式：列表不允许为空——删除后若为空则补一台占位设备（与
     * {@link ConfigService#load()} 的兜底一致）；落盘失败时回滚内存列表。
     *
     * <p>副作用：写盘（I/O）。
     */
    @FXML
     ...
     *
     * <p>副作用：写盘 {@code settings.properties}（I/O）；写盘失败仅提示，
     * 不影响本次主题在内存中生效。
     */
    @FXML
     ...
     *
     * <p>调用顺序约定：{@link #syncThemeButton(String)} 必须位于 scene 判空
     * 之前——initialize 阶段 scene 为 null，按钮图案需无条件同步，而样式表
     * 操作要求场景已就绪（FxmlLoadTest 有回归断言）。
     *
     ...
     * 将设备字段回填表单并清除脏标记。
     *
     * <p>回填期间置 {@link #suppressChangeEvents}，避免 setText 触发 dirty；
     * 之后 {@link #currentDevice} 指向该实例。
     *
     * @param device 目标设备
     */
     ...
     *
     * @return 新设备实例，字段来自表单
     * @throws IllegalArgumentException 端口不在 1-65535 或 MAC 格式非法时
     *         （消息可直接展示，见 {@link #applyFormTo(Device)}）
     */
     ...
     *
     * <p>数据契约：设备与设置分文件写入 {@code device.properties} /
     * {@code settings.properties}；连发次数解析失败时回退
     * {@link AppConfig#DEFAULT_SEND_COUNT}。
     *
     ...
     *
     * <p>约定：-1 为「非法」哨兵值，由调用方决定提示文案；合法区间参数
     * 均来自 {@link AppConfig}。
     *
