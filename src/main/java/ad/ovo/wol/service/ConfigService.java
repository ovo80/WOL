 ...
 *
 * <p>文件布局（目录由系统属性 {@code wol.config.dir} 覆盖，默认
 * {@code ~/.wol}，Windows 为 {@code C:\Users\<用户名>\.wol}）：
 * <ul>
 *   <li>{@code device.properties}（UTF-8）：设备列表，键
 *       {@code device.N.name|mac|broadcast|port}（N 从 1 起）；兼容旧单设备键
 *       {@code device.mac|broadcast|port}</li>
 *   <li>{@code settings.properties}（UTF-8）：软件设置，键 {@code ui.theme}
 *       （dark|light）与 {@code device.count}（1-100）</li>
 * </ul>
 *
 * <p>迁移链（首次加载自动执行，失败仅告警不阻塞）：程序目录旧配置 → 新配置
 * 目录；单文件配置 → 设备/设置双文件拆分。
 *
 * <p>写入语义：同目录临时文件 + 原子改名
 * （{@link #writeAtomically(Path, ContentWriter)}），任一步失败不破坏既有文件。
 *
 * <p>线程安全：静态无状态，可并发调用。
 */
public final class ConfigService {
     ...
     *
     * <p>容错契约：文件不存在或读取失败时返回「含一台默认设备的配置」——
     * 调用方永远得到非空列表，不因配置问题阻断启动。
     *
     ...
     *
     * <p>容错契约：非法键值回退默认（次数 {@link AppConfig#DEFAULT_SEND_COUNT}、
     * 主题 {@link AppConfig#DEFAULT_THEME}）。
     *
    ...
    public static AppSettings loadSettings() {
        ...
        if (Files.exists(file)) {
            ...
            try (InputStream in = Files.newInputStream(file)) {
                props.load(new InputStreamReader(in, StandardCharsets.UTF_8));
                settings.setSendCount(parseRange(props.getProperty(KEY_COUNT), AppConfig.DEFAULT_SEND_COUNT,
     ...
     *
     * <p>数据契约：键 {@code device.N.name|mac|broadcast|port}（N 从 1 连续编号，
     * 与列表下标对应）；port 为十进制字符串。值仅转义反斜杠——'=' 出现在值中
     * 是安全的（Properties 按首个 '=' 切分键值），换行由单行输入框排除。
     *
     ...
     *         见 {@link Device} 与 {@link AppSettings}）
     * @throws IOException 写入失败时
     */
     ...
     *
     * <p>触发条件：新目录文件不存在，且程序目录（JAR/classes 所在目录）存在
     * 旧配置文件时；仅拷贝不删除旧文件，回滚安全。
     *
     ...
     *
     * <p>触发条件：设置文件不存在且设备文件含 {@code ui.theme} 或
     * {@code device.count} 键时；设备文件中的设备键不受影响。
     *
    ...
    private static void migrateSettingsFromLegacyFile(Path settingsFile) {
        ...
        try {
            ...
            settings.setSendCount(parseRange(count, AppConfig.DEFAULT_SEND_COUNT,
                    AppConfig.SEND_COUNT_MIN, AppConfig.SEND_COUNT_MAX, "发送次数"));
     ...
     * <ul>
     *   <li>{@code device.N.name} → String 设备名</li>
     *   <li>{@code device.N.mac} → String MAC（如 00:1A:2B:3C:4D:5E）</li>
     *   <li>{@code device.N.broadcast} → String 广播地址/主机名</li>
     *   <li>{@code device.N.port} → String 十进制端口；非法回退默认 9</li>
     * </ul>
