 ...
 *
 * <p>异常契约：对外只抛 {@link WolException}；底层校验异常
 * （{@link IllegalArgumentException}）与网络异常（{@link IOException}、
 * {@link InterruptedException}）均在本类转译，调用方只需处理一种异常类型。
 *
 * <p>副作用：UDP 网络发送（I/O）；阻塞调用——连发 N 次耗时约为
 * N × 100ms（见 {@link AppConfig#SEND_INTERVAL_MS}），调用方应置于后台线程。
 *
 * <p>线程安全：无实例字段，可多线程共享。
 */
public class WolService {
     ...
     * 按设备配置发送连发魔术包。
     *
     * @param device 目标设备；mac 需为合法 MAC，broadcast 需可解析，
     *         port 需在 1-65535 之间
     * @param count 连发次数，取值 {@link AppConfig#SEND_COUNT_MIN} 至
     *         {@link AppConfig#SEND_COUNT_MAX}
     ...
     * @param broadcast 广播地址：IPv4（如 10.0.0.255）、IPv6 或主机名；
     *         不允许携带协议前缀（http:// 等）
     * @param port 目标端口，1-65535
     * @param count 连发次数，1-100
     * @throws WolException 校验失败（消息含具体原因）或网络发送失败/中断时；
     *         中断路径会恢复线程中断标志
     */
     ...
     * 校验连发次数范围。
     *
     * @throws IllegalArgumentException 次数不在 {@link AppConfig#SEND_COUNT_MIN} 至
     *         {@link AppConfig#SEND_COUNT_MAX} 区间时
     */
     ...
     *
     * <p>分支规则：含 {@code "://"} 判定为误带协议前缀；含 {@code ":"} 按 IPv6
     * 字符集校验（语义校验交由 {@link WolUtil#resolveAddress(String)}）；
     * 纯数字与点按 IPv4 逐段校验（4 段、每段 0-255）；其余按主机名规则
