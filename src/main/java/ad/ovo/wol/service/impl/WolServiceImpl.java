...
/** {@link WolService} 默认实现：UDP 单播/广播发送与 SRV 解析链路（无实例状态，可多线程共享）。 */
public class WolServiceImpl implements WolService {
  ...
  public void sendWakeUp(Device device, int count) throws WolException {
    if (device.isSrvEnabled()) {
      sendWakeUpViaSrv(device.getMacAddress(), device.getSrvName(), count);
      return;
    }
  ...
  @Override
  public String sendWakeUpViaSrv(String mac, String srvName, int count) throws WolException {

    // 校验与解析先行完成：不合法参数不进入 UDP 发送
    final byte[] packet;
    final SrvUtil.SrvRecord record;
    final InetAddress address;
    try {
      packet = WolUtil.buildMagicPacket(WolUtil.parseMac(mac));
      validateCount(count);
      record = SrvUtil.resolve(srvName);
      address = record.resolveTarget();
      WolUtil.validatePort(record.getPort());
    } catch (IllegalArgumentException e) {
      throw new WolException(e.getMessage(), e);
    }

    try (DatagramSocket socket = WolUtil.createBroadcastSocket()) {
      sendLoop(socket, packet, address, record.getPort(), count, record.display());
    } catch (IOException e) {
      log.error("UDP Socket 创建失败: srv={}, target={}", srvName, record.display(), e);
      throw new WolException("网络发送失败：" + e.getMessage(), e);
    }

    String display = address.getHostAddress() + ":" + record.getPort();
    log.info(
        "SRV 唤醒请求完成: mac={}, srv={}, target={} -> {}, count={}",
        mac,
        srvName,
        record.display(),
        display,
   ...
   * @param port 目标端口
   * @param count 连发次数
   * @param targetLabel 日志用目标描述（广播地址或 SRV 展示文本）
