 ...
 *
 * <p>异常契约：参数错误抛 {@link IllegalArgumentException}（消息面向用户，
 * 可直接展示）；网络失败抛 {@link IOException}（由 Service 层转译）。
 *
...
public final class WolUtil {
     ...
     * 解析 MAC 字符串为 6 字节数组。
     *
     * <p>数据契约：入参支持 ":" 或 "-" 分隔的 6 组两位十六进制
     * （如 {@code 00:1A:2B:3C:4D:5E}），大小写不限，容忍首尾空白，
     * 分隔符可混用。
     *
     * @param mac MAC 字符串；null 或空白视为非法
     * @return 6 字节数组（字节序与 MAC 原文一致）
     * @throws IllegalArgumentException 组数不为 6、任一组非两位十六进制时；
     *         消息含具体出错位置
     */
     ...
     *
     * <p>WOL 协议格式：网卡识别到 6 字节前缀后，读取后续 16 份相同的 MAC，
     * 匹配自身地址即触发唤醒。
     *
     ...
     *
     * <p>判定规则：IPv4 末段为 255，或 IPv6 首字节为 0xFF，视为标准广播；
     * 不满足且非本机任意地址（0.0.0.0/::）时仅记录警告、不阻止发送——
     * 单播唤醒等合法目标不应被误杀。不用
     * {@code InetAddress.isBroadcastAddress()}：部分 JDK 17 构建缺失该方法
     * 实现，改以字节级判定。
     *
     * @param broadcast IPv4/IPv6 地址或主机名（主机名形态触发 DNS 解析）
     * @return 解析后的地址
     * @throws IllegalArgumentException 入参为 null/空白，或解析失败时；
     *         消息含原始地址
     */
     ...
     * 创建广播发送用 UDP Socket。
     *
     * <p>配置：SO_BROADCAST 开启（允许发往广播地址）；SO_TIMEOUT 2000ms 为
     * 防御性设置——当前发送路径不读取数据，仅防极端情况下悬挂。
     *
     ...
     * 向目标地址发送单个魔术包。
     *
     * @param socket 发送用 Socket（来自 {@link #createBroadcastSocket()}，
     *         由调用方持有并负责关闭）
